...
class Moderation(commands.Cog, name="Moderation"):
    ...
    async def timeout(
        ...
        if member is None or duration is None:
            await ctx.info(
                "*Use: `timeout <user> <duration>` | `timeout list` | `timeout remove <user>`*"
            )
            return
        import re as _re
    ...
    async def forcenick(
        ...
        *,
        ...
        if member is None or nickname is None:
            await ctx.info(
                "*Use: `forcenick <user> <nickname>` or `forcenick cancel <user>`*"
            )
            return
        if len(nickname) > 32:
            await ctx.err("*Nickname must be 32 characters or less.*")
    ...
    async def picperms(self, ctx: "PushieContext", member: discord.Member) -> None:
        ...
        await ctx.ok(f"*Image permissions **{status}** for {member.mention}.*")
    ...
    async def picperms_role(self, ctx: "PushieContext", role: discord.Role) -> None:
        ...
        await ctx.ok(f"*Pic perms role set to {role.mention}.*")
    ...
    async def mod_setup(self, ctx: "PushieContext") -> None:
        ...
        try:
            ...
            if not jail_cat:
                ...
                jail_cat = await ctx.guild.create_category(
        ...
        for role_attr, perm_kwargs in [
            ...
            for ch in ctx.guild.text_channels:
                try:
                    ...
                    pass
        await ctx.ok(f"*Synced `{synced}` channel permission(s).*")
    ...
    async def _set_invoke(
        self, ctx: "PushieContext", action: str, delivery: str, message: str
    ) -> None:
        assert ctx.guild is not None
        g = await ctx.bot.storage.get_guild(ctx.guild.id)
    ...
    async def invoke_jail(
        self, ctx: "PushieContext", delivery: str = "channel", *, message: str = ""
    ) -> None:
    ...
    async def invoke_list(self, ctx: "PushieContext") -> None:
        ...
        await ctx.send(
            embed=UI.info(f"**Invoke Messages**\n\n" + "\n".join(lines))
        )
    ...
    async def strip(self, ctx: "PushieContext", member: discord.Member) -> None:
        ...
        try:
            ...
            await ctx.ok(
                f"*Stripped `{len(roles_to_remove)}` moderation role(s) from {member.mention}.*"
            )
        except discord.Forbidden:
    ...
    async def nsfw(
        ...
        try:
            ...
            await ctx.ok(f"*NSFW {state} for {ch.mention}.*")
        except discord.Forbidden:
    ...
    async def lockdown(self, ctx: "PushieContext") -> None:
        ...
        for ch in ctx.guild.text_channels:
            try:
                ...
                pass
        await ctx.ok(f"*Server locked down. (`{locked}` channels locked)*")
    ...
    async def lockdown_staff(self, ctx: "PushieContext", role: discord.Role) -> None:
        ...
        await ctx.ok(f"*{role.mention} set as lockdown-exempt staff role.*")
    ...
    async def unlockdown(self, ctx: "PushieContext") -> None:
        ...
        for ch in ctx.guild.text_channels:
            try:
                ...
                pass
        await ctx.ok(f"*Server unlocked. (`{unlocked}` channels restored)*")
